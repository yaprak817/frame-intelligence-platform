...
function ResolvedResultView({ jobId, downloadError }: { jobId: string; downloadError: string | null }) {
  ...
  return <><div className="annotation-entry"><a className="button primary" href={`/jobs/${encodeURIComponent(jobId)}/annotations`}>Etiketlemeye başla</a></div>{dataset ? <DatasetGallery jobId={jobId} initialDownloadError={downloadError} /> : <ResultGallery jobId={jobId} />}</>;
}
