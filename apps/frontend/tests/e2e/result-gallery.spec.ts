...
async function mockResult(page: Page, frameCount = 1) {
  await page.route(`**/api/v1/jobs/${jobId}`, (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ id: jobId, status: "SUCCEEDED", source_type: "UPLOAD", source: "clip.mp4", created_at: "2026-08-24T10:00:00Z", started_at: "2026-08-24T10:00:01Z", completed_at: "2026-08-24T10:00:03Z", failure: null, result: { available: true, metadata_url: `/api/v1/jobs/${jobId}/result`, manifest_download_url: `/api/v1/jobs/${jobId}/result/manifest` } }) }));
